...
{
    //匹配电话号码
    NSString *regex_phonenum = @"\\d{2}-\\d{8}|\\d{3}-\\d{7}|\\d{4}-\\d{6}|\\d{4}-\\d{3}-\\d{3}|\\d{4}-\\d{7}|\\d{4}-\\d{6}|\\d{10}|\\d{9}|\\d{8}";
...
+ (NSString *)transformString:(NSString *)originalStr emojiDic:(NSDictionary *)_emojiDic
{
    ...
    NSString *regex_emoji = @"\\([a-zA-Z0-9\\u4e00-\\u9fa5\\s]+\\)";
    ...
    if ([array_emoji count])
    {
        for (NSString *str in array_emoji)
        {
            ...
            if (i_transCharacter)
            {
                NSString *imageHtml = [NSString stringWithFormat:@"<img src='%@' width='20' height='20'>", i_transCharacter];
